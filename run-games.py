...
# Make API use optional?
# OpenAI API
import openai
from openai import OpenAI
client = OpenAI(api_key=os.getenv('OPENAI_API_KEY'), organization=os.getenv('OPENAI_ORGANIZATION'))
openai_models = [dict(item)['id'] for item in list(client.models.list())]

# Anthropic API
from anthropic import Anthropic, HUMAN_PROMPT, AI_PROMPT
anthropic = Anthropic(api_key=os.getenv('ANTHROPIC_API_KEY'))
# This is temporary. Better to get up-to-date list of anthropic models to do the check
anthropic_models = ['claude-2.1', 'claude-2.0', 'claude-instant-1.2']
...
def get_prompts(player_name, opponent_name, role, persona, end_prob, history_str, matrix, labels_row, labels_column, message_round, total_message_rounds, strategic_reasoning, sys_prompt_substitutions = {}):
	...
	user = f"Here is a summary of the history of play thus far:\n{history_str}.\n\n{final_message_round} Now please output a correctly-formatted JSON. Output only the JSON and no other text:"
...
def get_response_openai(player_name, opponent_name, role, persona, end_prob, history_str, matrix, labels_row, labels_column, message_round=None, total_message_rounds=None, strategic_reasoning=False, sys_prompt_substitutions={}, model='gpt-4-32k', temperature = 1.0, **kwargs):
	...
	if (model == 'gpt-4-1106-preview') or (model == 'gpt-4-turbo-preview') or (model == 'gpt-4-0125-preview'):
		kwargs['response_format']={ "type": "json_object" }
 
	for _ in range(max_retries):
		try:
			completion = client.chat.completions.create(model=model, messages=[{'role': 'system', 'content': system, }, {'role': 'user', 'content': user}], temperature=temperature, **kwargs)
	...
	if not answer_dict:
		...
		logging.error('Failed to get response from OpenAI API even after multiple retries')
 
	return answer_dict

# Claude refuses to answer so this is not used
def get_response_anthropic(player_name, opponent_name, role, persona, end_prob, history_str, matrix, labels_row, labels_column, message_round=None, total_message_rounds=None, strategic_reasoning=False, sys_prompt_substitutions={}, model='claude-2.1', temperature = 1.0, **kwargs):
	system, user = get_prompts(player_name, opponent_name, role, persona, end_prob, history_str, matrix, labels_row, labels_column, message_round, total_message_rounds, strategic_reasoning=strategic_reasoning, sys_prompt_substitutions=sys_prompt_substitutions)
	max_retries = 5
	retry_interval_sec = 20
	answer_dict = {}
	...
	for _ in range(max_retries):
		try:
			completion = anthropic.beta.messages.create(
				model=model,
			...
			if completion.stop_reason != "end_turn":
				raise Exception("Completion error: stop_reason is not end_turn")
			answer = completion.content[-1].text
			print("ANSWER: ", answer)
			answer_dict = json.loads(answer)
	...
	if not answer_dict:
		# If it does not succeed after multiple retries, consider this a failed game and log it
		logging.error('Failed to get response from Anthropic API even after multiple retries')
 
	return answer_dict

# https://github.com/TheBlokeAI/dockerLLM/blob/main/README_Runpod_LocalLLMsUIandAPI.md
def get_response_runpod(player_name, opponent_name, role, persona, end_prob, history_str, matrix, labels_row, labels_column, message_round=None, total_message_rounds=None, strategic_reasoning=False, sys_prompt_substitutions={}, model='gpt-4-32k', temperature = 1.0, **kwargs):
	pass
...
def get_response(player_name, opponent_name, role, persona, end_prob, history_str, matrix, labels_row, labels_column, message_round=None, total_message_rounds=None, strategic_reasoning=False, sys_prompt_substitutions={}, model='gpt-4-32k', temperature=1.0, **kwargs):
	...
	# Use different get_response_* functions for different models
	...
	if model in anthropic_models:
		return get_response_anthropic(player_name, opponent_name, role, persona, end_prob, history_str, matrix, labels_row, labels_column, message_round=message_round, total_message_rounds=total_message_rounds, strategic_reasoning=strategic_reasoning, sys_prompt_substitutions=sys_prompt_substitutions, model=model, temperature = temperature, **kwargs)

	# if none of the above, assume it is a open-source model
